...
main() {
	char option, _D1[] = "Day", _W[] = "Week", _M[] = "Month", _Y[] = "Year";
	int i, period;
	...
	do {
		...
		switch(option) {
			case 'D':
				...
				displayTotAvg(period, totIncome, _D);        
				break;
			case 'W':
				period = getPeriod(_W);
				break;
...
displayTotAvg(int period, float totIncome, char *whatPeriod) {
	float avgIncome;
	printf("\nTotal income within %d %ys: %.2f\n\n", period, whatPeriod, totIncome);                              
	printf("Average %sily Income: %.2f\n\n\n", whatPeriod, totIncome / period);
}
