	...
	do {
		...
		switch(option) {
			case 'D':
				...
				/* ==== get income amounts per day and validates the value ==== */
				for(i = 1; i <= period; i++) {
					...
					do {
						...
						/* ==== continuesly scans amount until the user inputs a valid value ====*/
					} while (isalpha(income));
					//} while (income < 0 || isalpha(income) || getchar() != '\n');
