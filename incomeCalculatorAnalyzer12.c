...
int main(){
	...
	/* ==== runs the program until the user wishes to exit ==== */
	do{
		/* ==== displays the menu until the user chooses a valid option ==== */
		do{
		  ...
		  /* ==== redisplays the menu until the user chooses a valid option ==== */
		} while(option != 'D' && option != 'W' && option != 'M' && option != 'Y' && option != 'Q');
		
		switch(option) {
			
			/* ==== executes when the user chooses option D ==== */
			case 'D': 
				do {
					printf("\n\nNumber of Days: ");
					scanf("%d", &period);
